...
var amity = {
	 ...
	 */
	setConfigStores: function(storeDef){
		var self = this;
	...
	use: function(adapter){
		...
		// In the case of replica sets, only the first hostname is used as the key.
		this.adapters[adapter.namespace] = adapter;
...
var serverService = {
	...
	get: function(method, params, callback) {
		...
		if (method === 'status') {
			...
			// If an adapter exists with this hostname...
			if (amity.adapters[hostname]) {
				// And it has a getStatus method...
				if (amity.adapters[hostname].getStatus) {
					amity.adapters[hostname].getStatus(function(status){
						var data = {
							hostname: hostname,
							status:status
						};
