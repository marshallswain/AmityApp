...
var amity = {
	...
	use: function(adapter){
		...
		// console.log(adapter);
		// Make sure the adapter is an object.
		if (typeof adapter === 'object') {
			// Get the object's keys.
			var keys = Object.keys(adapter);
			// Loop through them.
			for (var i = keys.length - 1; i >= 0; i--) {
				// If the key starts with 'amity_'...
				if (keys[i].indexOf('amity_') === 0) {
					// It's possibly a group of services that are ready for setting up.
					var amityGroup = adapter[keys[i]];
					// console.log(amityGroup);
					// If it's not an empty group...
					if (amityGroup.length !== 0) {
						// ...loop through the array
						// for (var n = amityGroup.length - 1; n >= 0; n--) {
							// Assemble the full endpoint address.
							// var endpoint = this.apiPrefix + '/' + adapter.namespace + '/' + amityGroup[n].name;
							// console.log(endpoint);
							// console.log(amityGroup[n]);
							//

							// and use it to register the Feathers service.
							// this.app.use(endpoint, adapter[amityGroup][n].service);

							if (index == 0) {
								console.log('todos')
								this.app.use('/api/todos', require('./services/todos'));
							} else {
								console.log('tasks')
								this.app.use('/api/tasks', require('./services/todos'));
							}
							index++;
							//
							console.log('hi');
